...
const expectedOrder=['dashboard','hosting','accounts','sites','webtools','schedules','databases','files','security','backups'];
const firstLinks=await page.locator('#nav a').evaluateAll(nodes=>nodes.slice(0,10).map(n=>n.getAttribute('href')?.replace('#','')));
