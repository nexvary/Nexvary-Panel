...
const expectedOrder=['dashboard','hosting','accounts','mail','sites','webtools','schedules','databases','files','security','backups'];
const firstLinks=await page.locator('#nav a').evaluateAll(nodes=>nodes.slice(0,11).map(n=>n.getAttribute('href')?.replace('#','')));
