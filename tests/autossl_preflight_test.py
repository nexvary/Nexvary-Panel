...
with tempfile.TemporaryDirectory(prefix="nvp-autossl-preflight-") as tmp:
    ...
    conf.write_text("server { listen 80; server_name example.com shop.example.com; root /var/www/example.com; }\n", encoding="utf-8")

    entry.core._site_root = lambda domain: root
    ...
    assert len(probes) == 2
    assert not any((root / ".well-known" / "acme-challenge").glob("nexvary-autossl-*"))

    conf.write_text("server { listen 80; server_name example.com; root /var/www/example.com; }\n", encoding="utf-8")
    result = entry._autossl_preflight({"domain": "example.com", "domains": ["shop.example.com"]})
