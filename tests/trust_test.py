...
all_services = {"nginx": True, "mariadb": True, "fail2ban": True, "crowdsec": False, "ssh": True, "docker": True}
...
crowdsec_only = _trust_posture(
    enabled_2fa=True,
    services={"nginx": True, "mariadb": True, "fail2ban": False, "crowdsec": True},
    backups_count=1,
    disk_percent=55,
    critical_unread=0,
)
assert next(item for item in crowdsec_only["checks"] if item["id"] == "active-defense")["ok"] is True
assert crowdsec_only["score"] == 100

weak = _trust_posture(enabled_2fa=False, services={}, backups_count=0, disk_percent=96, critical_unread=3)
assert 0 <= weak["score"] <= 100
assert weak["score"] == 15
assert weak["level"] == "risk"
assert all(item["view"] in EXPECTED_VIEWS for item in weak["checks"])

mixed = _trust_posture(
    enabled_2fa=True,
    services={"nginx": True, "mariadb": True, "fail2ban": False, "crowdsec": False},
    backups_count=1,
    disk_percent=70,
    critical_unread=0,
)
...
assert next(item for item in mixed["checks"] if item["id"] == "active-defense")["view"] == "services"
assert next(item for item in mixed["checks"] if item["id"] == "backup")["view"] == "backups"
print("Nexvary Panel operational trust score/guided-fix/active-defense tests: PASS")
