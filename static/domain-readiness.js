(()=>{
  ...
  if(!select||!refresh||!score||!grade||!meta||!list)return;
  ...
  async function init(){
    try{const d=await api('/api/domain-health');const rows=d.domains||[];select.replaceChildren(option('','اختر نطاقًا…'));rows.forEach(r=>select.append(option(r.domain,r.owner&&r.owner!=='admin'?`${r.domain} · ${r.owner}`:r.domain)));if(rows.length){select.value=rows[0].domain;render(rows[0])}else clear()}
    catch(e){meta.textContent=`تعذر تحميل Domain Readiness: ${e.message}`}
  }
  select.addEventListener('change',loadDomain);refresh.addEventListener('click',loadDomain);init();
})();
