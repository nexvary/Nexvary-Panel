(()=>{
  ...
  function fieldChanged(){const custom=field.value==='header';headerWrap.hidden=!custom;header.required=custom;if(!custom)header.value='';}
  function actionChanged(){const a=action.value;if(a==='discard'){destination.value='';destination.disabled=true;destination.required=false;destination.placeholder='لا توجد وجهة';}else{destination.disabled=false;destination.required=true;destination.placeholder=a==='fileinto'?'Global':'archive@external.example';}}
  function updateControls(){const locked=!providerOnline||!featureAllowed||!domain.value;for(const el of [field,match,priority,pattern,action,enabled,save])el.disabled=locked;fieldChanged();actionChanged();if(locked){header.disabled=true;destination.disabled=true;}else{header.disabled=field.value!=='header';actionChanged();}state.textContent=!providerOnline?'PROVIDER OFFLINE':featureAllowed?'FOUNDATION · ACTIVE':'DISABLED BY PACKAGE';scope.textContent=owner?`${owner} · ${mailboxCount} MAILBOX`:'—';}
  ...
  $('#mailGlobalFilterForm').addEventListener('submit',async event=>{event.preventDefault();if(!providerOnline)return say('Mail Provider غير متصل.','error');if(!featureAllowed)return say('email.global_filters معطلة في حزمة الحساب.','error');const data=payload();const validation=validateClient(data);if(validation)return say(validation,'error');const id=Number(idField.value||0);save.disabled=true;say(id?'جاري تحديث Global Filter ومزامنة Mailboxes…':'جاري إنشاء Global Filter ومزامنة Mailboxes…');try{const result=await api(id?`/api/mail/global-filters/${id}`:'/api/mail/global-filters',{method:id?'PUT':'POST',body:JSON.stringify(data)});rows=result.filters||[];mailboxCount=Number(result.mailbox_count||mailboxCount);render();resetForm();say(id?'تم تحديث Global Filter ومزامنة الحساب بنجاح.':'تم إنشاء Global Filter وتطبيقه على الحساب.','ok');}catch(err){say(safeStepError(err),'error');}finally{updateControls();}});
