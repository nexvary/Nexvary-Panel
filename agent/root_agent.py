...
def reply(conn,obj): conn.sendall((json.dumps(obj,separators=(",",":"))+"\n").encode())
def valid_domain(value): return isinstance(value,str) and bool(DOMAIN_RE.match(value))
def run(args,timeout=120,stdin=None):
    proc=subprocess.run(args,capture_output=True,text=True,input=stdin,timeout=timeout,env={"PATH":"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin","LANG":"C.UTF-8","HOME":"/root","GIT_CONFIG_NOSYSTEM":"1"})
    if proc.returncode:return {"ok":False,"error":(proc.stderr or proc.stdout or "operation failed")[-4000:]}
    return {"ok":True,"output":(proc.stdout or "")[-12000:]}

def _safe_extract(tf: tarfile.TarFile, destination: Path) -> None:
    """Extract a trusted-size archive while rejecting links, devices and path traversal."""
    destination=destination.resolve()
    members=tf.getmembers()
    total=0
    for member in members:
        name=member.name
        if not name or "\x00" in name or "\\" in name: raise ValueError("unsafe archive member")
        rel=PurePosixPath(name)
        if rel.is_absolute() or any(part in {"", ".", ".."} for part in rel.parts): raise ValueError("unsafe archive path")
        if member.issym() or member.islnk() or member.isdev() or member.isfifo(): raise ValueError("archive links/devices are blocked")
        if not (member.isdir() or member.isfile()): raise ValueError("unsupported archive member")
        total += max(0, member.size)
        if total > MAX_WP_ARCHIVE * 4: raise ValueError("archive expands beyond safety limit")
        target=(destination.joinpath(*rel.parts)).resolve()
        if os.path.commonpath((str(destination),str(target))) != str(destination): raise ValueError("archive path escapes destination")
...
def _maintenance(action:str):
    ...
    if not plan:return {"ok":False,"error":"maintenance action not allowed"}
    pre,command,post,timeout=plan; before=run(pre,timeout=min(timeout,20))
    if not before.get("ok"):return {"ok":False,"phase":"precheck","error":before.get("error","precheck failed")}
    changed=run(command,timeout=timeout)
    if not changed.get("ok"):return {"ok":False,"phase":"apply","error":changed.get("error","operation failed")}
    after=run(post,timeout=min(timeout,20))
    if not after.get("ok"):return {"ok":False,"phase":"postcheck","error":after.get("error","postcheck failed"),"rollback":"manual-review-required"}
...
def _safe_path(domain,rel,allow_root=False,allow_missing_leaf=False):
...
def file_list(domain,rel):
    ...
    for entry in sorted(path.iterdir(),key=lambda p:(not p.is_dir(),p.name.lower()))[:500]:
        try:
            st=entry.lstat();kind="blocked-link" if stat.S_ISLNK(st.st_mode) else "dir" if stat.S_ISDIR(st.st_mode) else "file" if stat.S_ISREG(st.st_mode) else "blocked";rows.append({"name":entry.name,"kind":kind,"size":st.st_size if kind=="file" else 0,"mtime":int(st.st_mtime)})
        except OSError:pass
...
def file_write(domain,rel,content):
    ...
    try:data=content.encode();os.write(fd,data);os.fsync(fd);uid,gid=_www_ids();os.fchown(fd,uid,gid);os.fchmod(fd,0o640)
    finally:os.close(fd)
...
def file_delete(domain,rel):
    _,p=_safe_path(domain,rel);p.rmdir() if p.is_dir() else p.unlink();return {"ok":True}
...
def handle(req):
    ...
    if action=="docker-control":
        c,d=req.get("container",""),req.get("desired","");return run(["/usr/local/sbin/nvpctl","docker-control",c,d],40) if isinstance(c,str) and CONTAINER_RE.match(c) and d in {"start","stop","restart"} else {"ok":False,"error":"invalid docker request"}
    if action=="file-list":
